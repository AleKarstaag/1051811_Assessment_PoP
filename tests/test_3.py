"""Test Galerkin approximation of the Poisson problem."""
import pytest
import numpy as np
...
@pytest.mark.parametrize(
    "cells_value, length, origin,f,u", [
     (5, 100, [0, 0],
      lambda x, y: (2*np.pi**2/(100**2)) * np.sin(x*np.pi/100)
      * np.sin(y*np.pi/100),
     ...
     (5, 2, [-1, -1],
      lambda x, y: -(2*np.exp(x**2)+4*np.exp(x**2)*x**2)
      * (y**2-1)-2*(np.exp(x**2)-np.e),
